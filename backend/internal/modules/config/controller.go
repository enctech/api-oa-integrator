...
func InitController(e *echo.Group) {
	...
	g.GET("/snb-config", c.getAllSnBConfig)
	g.GET("/snb-config/:id", c.getSnBConfig)
	g.DELETE("/snb-config/:id", c.deleteSnbConfig, middlewares.AdminOnlyMiddleware())

	g.GET("/integrators", c.getIntegrators)
	g.GET("/integrator-config", c.getIntegratorConfigs)
	g.GET("/integrator-config/:id", c.getIntegratorConfig)

...
func (c Config) VerifyVehicle(plateNumber, entryLane string) error {
	...
	req.Header.Set("timestamp", time.Now().Format(time.RFC3339))
	req.Header.Set("clientId", c.ClientID.String)
	req.Header.Set("function", "falcon.device.status")
	req.Header.Set("version", viper.GetString("app.version"))
